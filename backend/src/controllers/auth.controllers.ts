...
import { sendResetPasswordEmail, sendResetSuccessEmail, sendVerificationEmail, sendWelcomeEmail } from "../nodemailer/emails";
...
export const verifyEmail = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user) {
      ResponseBuilder.send(res, HtttpStatus.BAD_REQUEST, false, registerMessages.wrong.invalidVerificationCode)
      return
    }
    ...
    ResponseBuilder.send(res, HtttpStatus.CREATED, true, registerMessages.wrong.emailVerifiedSuccesfully)
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      ResponseBuilder.send(res, HtttpStatus.BAD_REQUEST, false, registerMessages.wrong.userNotFound)
      return
    }
    const isPasswordCorrect = await bcrypts.compare(password, user.password)

    if (!isPasswordCorrect) {
      ResponseBuilder.send(res, HtttpStatus.BAD_REQUEST, false, registerMessages.wrong.invalidPassword)
      return
    }
    generateTokenAndSetCookie(res, user._id as mongoose.Types.ObjectId)
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    await sendResetPasswordEmail(user.email, `${config.clientUrl}/reset-password/${resetPasswordToken}`)

    ResponseBuilder.send(res, HtttpStatus.CREATED, true, registerMessages.success.resetPasswordEmailSent)
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      ResponseBuilder.send(res, HtttpStatus.BAD_REQUEST, false, registerMessages.wrong.invalidOrExpiredResetTokenPassword)
      return
    }
    const hashedPassword = await bcrypts.hash(password, 10)
    ...
    ResponseBuilder.send(res, HtttpStatus.OK, true, registerMessages.success.passwordResetSuccesfully)
...
export const checkAuth = async (req: RequestWithUserId, res: Response) => {
  try {
    ...
    if (!user) {
      ResponseBuilder.send(res, HtttpStatus.UNAUTHORIZED, false, registerMessages.wrong.unauthorized)
      return
    }
    ResponseBuilder.send(res, HtttpStatus.OK, true, registerMessages.success.logInSuccesfully, {
      user
